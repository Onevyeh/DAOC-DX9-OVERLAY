...
HRESULT WINAPI IDirect3D9Proxy::CreateDevice(
	UINT Adapter,
...
{
	// 1. Création du vrai device
	HRESULT hres = origIDirect3D9->CreateDevice(
		Adapter, DeviceType, hFocusWindow,
		BehaviorFlags, pPresentationParameters,
		ppReturnedDeviceInterface);

	if (SUCCEEDED(hres)) {
		// 2. Chargement des shaders custom
		LoadVegetationShader(*ppReturnedDeviceInterface);
		LoadLeavesShader(*ppReturnedDeviceInterface);

		// 3. Initialisation des modèles d’herbe HD (VertexBuffer/IndexBuffer custom)
		InitGrassModels(*ppReturnedDeviceInterface);

		// 4. Wrapping du device pour intercepter tous les appels
		IDirect3DDevice9Proxy* tmp = new IDirect3DDevice9Proxy(*ppReturnedDeviceInterface);

		// 5. Remplace le pointeur retourné par notre version proxy
