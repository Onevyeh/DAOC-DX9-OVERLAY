...
// Utilisation des structures sécurisées pour éliminer les warnings
// Remplace les déclarations directes de types D3DX sans initialisation par les versions sécurisées suivantes :
...
// Initialisations globales sûres
...
HRESULT IDirect3DDevice9Proxy::EndScene(void)
{
	...
	// (Optionnel : update global shader time ici si utilisé ailleurs)
...
HRESULT IDirect3DDevice9Proxy::SetTexture(DWORD Stage, IDirect3DBaseTexture9* pTexture)
{
	...
	if (pTexture && pTexture->GetType() == D3DRTYPE_TEXTURE)
	{
		...
		//Si hash déjà connu, on le récupère
		...
		// Mise à jour du hash courant (slot 0 seulement)
		if (Stage == 0)
		{
			g_lastTextureHash = texHash;

			// ------------------------------------------
			// DÉBUT de l'ajout LOG/DUMP VB VÉGÉTATION/FEUILLE
			// ------------------------------------------
			auto DumpVertexBufferAndLog = [&](const char* type, const std::set<uint32_t>& refSet) {
				if (refSet.find(texHash) != refSet.end() && g_detectedVegetationHashes.find(texHash) == g_detectedVegetationHashes.end()) {
					...
					g_frameLog << "[AUTO CAPTURE] Texture " << type << " détectée, Hash=0x"
